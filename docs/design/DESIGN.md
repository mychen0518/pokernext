...
- 「金底按鈕」（`Button/primary`）每個區塊最多一顆，其餘動作用 `Button/secondary`。區塊是一張卡片、作業頁的一欄、`PageHeader` 的動作區，或 Modal／Drawer 的動作列；所以 §3.2 作業頁每欄底部一顆、§4 `TodoCard` 每卡一顆都成立，但同一區塊裡不並排兩顆金底按鈕。玩家端手機另守 §3.1「一屏只放一個主要動作」。
