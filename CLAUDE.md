...
- **Planning / ticketing** (`/to-spec`, `/to-tickets`, `/grill-*`): any spec or ticket that touches a screen must name its surface (玩家端 / 合作端 / 管理端 / 接待端) and workspace (see `CONTEXT.md`), the page type, the reference image, and the DESIGN.md components it uses, in a `**UI:**` section (format in `docs/agents/issue-tracker.md`). Add at least one visual acceptance criterion per UI ticket.
