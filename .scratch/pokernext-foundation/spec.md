...
我們已經有一份 PRD、一份 spec、34 張票，以及一個六端可操作的高保真原型（`docs/design/prototype/pokernext-prototype.html`）。但 repo 裡還沒有任何正式程式碼，而且原型的三樣東西（視覺、狀態機、示範資料）目前只存在於一個單檔 HTML 裡，實作 agent 拿不到可 import 的版本。如果直接開始做第 01 票，每個 implementer 都會各自重抄一次 token、各自解釋一次「已入住」是什麼，六端很快就會長歪。
...
3. `tooling/demo`：示範工具鏈。`pnpm demo`（起 DB、灌 seed、啟 app、開發模式角色切換列）、`pnpm demo:script`（Playwright 依 13 步劇本跨六端操作並錄影）、`pnpm demo:diff`（正式頁 vs 原型頁截圖並排報告）。
...
2. As an implementer agent, I want `packages/domain` 提供 `nextStage()`、`depositAxes()`、`computeRefund()`、`TASK_TEMPLATES`，so that 六端投影同一份規則而不是各自推論。
3. As a reviewer agent, I want kitchen-sink 頁與原型截圖並排，so that 我能用視覺 diff 而不是主觀判斷來審 UI 票。
4. As the maintainer（Mingyao），I want `pnpm demo` 一個指令起整個系統並能用角色切換列在六端間跳，so that 每批票完成後我能親手走一遍。
...
- repo 形狀依 ADR-0001：pnpm workspace；`apps/web`、`packages/ui`、`packages/domain`、`packages/db`、`packages/ports`、`tooling/demo`。每個 package 依 `/setup-ts-deep-modules`：根目錄檔案是公開介面，`lib/`、`tests/` 私有。
...
- `packages/domain` 不依賴 React、Next、DB。狀態以 discriminated union 表達；轉移以純函式 `transition(state, event)` 表達；所有規則的期望值來自 PRD 8.2 與 6.6.3 的表，不從實作反推。
- 原型裡以下片段是「決策」，可精簡後 inline 到 domain 測試作為期望值來源（來自 prototype，非工作 demo）：
  - 行程階段：`applied → approved → confirmed → arrived → checked_in → checked_out → settled → archived`；旁路 `cancelling → cancelled`（只由天城完成事件觸發）。
...
- Seams（請維護者確認）：
  1. `packages/domain` 的根目錄函式（`transition`、`depositAxes`、`computeRefund`、`taskTemplate`）— 單元測試，最多的測試在這裡。
