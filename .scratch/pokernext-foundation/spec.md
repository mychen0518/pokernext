...
**Blocks:** 所有涉及畫面的票（00a）；每批收尾的 `pnpm demo`／`pnpm demo:script` 驗收（00b）
...
1. **00a `packages/ui`**：設計系統套件。token 與 `docs/design/DESIGN.md` §2 一致，元件名與 §4 一致；附一頁 kitchen-sink 供截圖比對。
2. **00b `tooling/demo`**：示範工具鏈。`pnpm demo`（起 DB、跑 migration、灌 seed、啟 app、開發模式角色切換列）、`pnpm demo:script`（Playwright 依 13 步劇本跨工作區操作並錄影）、`pnpm demo:diff`（正式頁 vs 原型頁截圖並排報告）。為了讓角色切換列走真實的 session 建立路徑，00b 同時建立最小的帳號與 session。
...
8. As the implementer of 第 03、10 票，I want 擴充 00b 已建立的帳號與 session，而不是另建一套，so that 系統只有一條 session 建立路徑。
...
- demo seed：
  - 00b 只 seed 六個工作區各一個 demo 帳號，名稱取自原型 `ROLES`（Alex Chen、琪琪、王經理、Mingyao、Amy、David Chen）。
...
- 示範劇本的 13 步以 Playwright 測試檔表達；00b 建立檔案與 13 個步驟，尚未有對應票的步驟標為 `test.fixme`，隨批次解除，同時作為批次 B／C 的端到端驗收。
...
- 拆票兩張：
  - 00a 設計系統套件：blocked by ADR-0001 accepted 與 repo 骨架，可與第 01 票並行。
  - 00b 示範工具鏈：blocked by 00a 與 `.scratch/pokernext-platform/issues/01`。
- 原本的「00b 領域核心」已刪除：它與第 20、21、22、27 票重複，期望值來自原型而非驗收條件，且違反 platform spec「核心規則須穿過 use-case 與資料庫測試」的決策。
